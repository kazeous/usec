...
type RegistrationItem = {
  ...
  members: Array<{ fullName: string; studentId: string; universityName: string; email: string; isCaptain: boolean }>;
};
          ...
          return (
            <article className="rounded-md border border-[#ded7ca] bg-[#fffdf8] p-4" key={registration.id}>
              ...
              <div className="mt-3 grid gap-1 text-sm sm:grid-cols-2">
                {registration.members.map((member) => <p key={member.studentId}><span className="font-bold">{member.fullName}</span> · {member.studentId} · {member.email}</p>)}
              </div>
